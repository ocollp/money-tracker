...
export default function CarGoal({ profiles, profile, months, appJwt, accessToken, price, onPriceChange }) {
  ...
  return (
    <div className="rounded-xl border border-violet-500/15 bg-white/[0.02] p-3 sm:p-4 space-y-4">
      ...
      <div>
        <label htmlFor="car-price" className="flex items-center justify-between gap-2 text-xs text-text-secondary">Preu total del cotxe <strong className="text-text-primary text-xl tracking-tight tabular-nums">{formatMoney(price)}</strong></label>
        <input id="car-price" type="range" min="18000" max="25000" step="500" value={price} onChange={e => onPriceChange(Number(e.target.value))} className="w-full h-10 accent-white/10 cursor-pointer" />
        <div className="flex justify-between text-xs text-text-secondary"><span>18.000 €</span><span>25.000 €</span></div>
      </div>
      <div className="grid grid-cols-1 min-[400px]:grid-cols-2 gap-3">
        {orderedPeople.map((person) => (
          <div key={person.owner} className="rounded-xl border border-white/[0.06] bg-black/10 p-3 sm:p-3.5 space-y-3 text-xs">
            <p className="flex items-center justify-between font-semibold text-sm">{person.owner}<span className="rounded-full bg-violet-400/10 px-2 py-0.5 text-[11px] font-medium text-violet-200">{person.share * 100}%</span></p>
            <p className="flex justify-between gap-2 text-text-secondary">Part a pagar <span className="text-text-primary tabular-nums">{formatMoney(person.payment)}</span></p>
            <p className="flex justify-between gap-2 text-text-secondary">Estalviat <span className="text-emerald-300 tabular-nums">{person.balance == null ? '—' : money(person.balance)}</span></p>
            {person.balance != null ? <>
